...
-(NSArray *)parseData:(NSData *)data{//Array of NSManagedObjects Papers, quiza solo al anadir
    ...
    HTMLNode *resultsByCategory = [groupmodule firstObject];//objectAtIndex:0];
		...
		if ([aes count]>0) {
			...
			for (HTMLNode *nameOfProduct in aes){
				...
				if ([nameOfProductString length]>0 && x == 0) {
					if([nameOfProduct getAttributeNamed:@"href"])[allLinks addObject:[nameOfProduct getAttributeNamed:@"href"]];
...
-(NSArray *)parseProduct:(NSData *)data{//Array of NSManagedObjects Papers, quiza solo al anadir
	...
	for (HTMLNode *li in lis) {
		...
		if ([pair count] > 1) {
			...
			if ([[[pair objectAtIndex:1]rawContents]length]>0) {
				...
				if ([aArray count]>0) {
					if([[aArray lastObject] contents])[theValues addObject:[[aArray lastObject] contents]];
				}else{
					if([[pair objectAtIndex:1]contents])[theValues addObject:[[pair objectAtIndex:1]contents]];
				}
			}else {
				[theValues addObject:@" "];
			}
    ...
    if (link.count > 0) {
        ...
        HTMLNode *linker = link.firstObject;
        if([linker getAttributeNamed:@"href"])[theValues addObject:[linker getAttributeNamed:@"href"]];
    }
