...
-(void)searchButton{
	if (!searching) {
		if ([self.searchBox.text length]>0) {
            ...
            [General showOKAlertWithTitle:@"Enter search terms" andMessage:nil delegate:self];
		}
	}else{
		searching = NO;
    }
...
-(void)goToAdvancedSearch{
	ADBPubmedDetailViewController *aspVC = [[ADBPubmedDetailViewController alloc]init];
    aspVC.delegate = self;
