...
-(NSManagedObjectContext *)managedObjectContext{
    ...
    return delegateApp.managedObjectContext;
}
...
-(void)processIncomingLoginData:(NSData *)data{
    [[IPImporter getInstance]processDataDirectly:data toObjectClasses:@[PERSON_DB_CLASS, ZGROUPPERSON_DB_CLASS, GROUP_DB_CLASS] usingPK:@[@"perPersonId", @"gpeGroupPersonId", @"grpGroupId"] erasingOld:@[[NSNumber numberWithBool:YES],[NSNumber numberWithBool:YES],[NSNumber numberWithBool:NO]] withCompletionBlock:^{
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *error){
        if(error){
            ...
            if ([[[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding]isEqualToString:@"0"]) {
                [General showOKAlertWithTitle:@"Alert" andMessage:@"Wrong email address or password"];
                return;
            }
            if ([[[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding]isEqualToString:@"-1"]) {
                [General showOKAlertWithTitle:@"Alert" andMessage:@"You account has not yet been activated"];
                return;
            }
            ...
            if (array.count > 0) {
                ...
                if (token) {
                    ...
                    [General showOKAlertWithTitle:@"Error" andMessage:@"There was an error with the server authentication"];
                    return;
                }
            }else{
                [General showOKAlertWithTitle:@"Error" andMessage:@"There was an error with the server authentication"];
...
-(void)invite:(NSString *)email{
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *error){
        if (error) {
            ...
            [General showOKAlertWithTitle:@"Invitation sent" andMessage:nil];
...
-(void)createAccountWithEmail:(NSString *)login andPassword:(NSString *)password{
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *error){
        ...
        if(error){
            ...
            [[NSNotificationCenter defaultCenter]postNotificationName:@"NSNC_CREATED" object:nil userInfo:[NSDictionary dictionaryWithObject:data forKey:@"json"]];
        }
    }];
}
-(void)requestNewPassword:(NSString *)email{
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *error){
        if(error)[General logError:error];
            ...
            [General showOKAlertWithTitle:@"Email sent" andMessage:@"You will get an email with your new password"];
