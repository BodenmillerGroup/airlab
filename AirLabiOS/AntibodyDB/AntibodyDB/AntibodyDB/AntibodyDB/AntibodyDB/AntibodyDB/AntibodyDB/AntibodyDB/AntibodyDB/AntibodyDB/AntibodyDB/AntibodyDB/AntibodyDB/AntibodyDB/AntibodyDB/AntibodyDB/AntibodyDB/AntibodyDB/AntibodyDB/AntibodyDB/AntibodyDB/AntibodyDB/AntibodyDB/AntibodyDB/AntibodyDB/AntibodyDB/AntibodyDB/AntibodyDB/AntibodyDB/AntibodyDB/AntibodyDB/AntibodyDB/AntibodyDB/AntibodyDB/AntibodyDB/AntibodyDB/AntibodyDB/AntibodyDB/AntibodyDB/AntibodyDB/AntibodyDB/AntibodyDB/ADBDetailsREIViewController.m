...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch (indexPath.row) {
        case 0:
        {
            cell.textLabel.text = [NSString stringWithFormat:@"Requested by: %@ %@", _rei.requester.person.perName, _rei.requester.person.perLastname];
            cell.detailTextLabel.text = _rei.reiRequestedAt;
        }
            break;
        case 1:
        {
            cell.textLabel.text = [NSString stringWithFormat:@"Approved by: %@ %@", _rei.approver.person.perName, _rei.approver.person.perLastname];
            //cell.detailTextLabel.text = _rei.reia;
        }
            break;
        case 2:
        {
            cell.textLabel.text = [NSString stringWithFormat:@"Ordered by: %@ %@", _rei.orderer.person.perName, _rei.orderer.person.perLastname];
            cell.detailTextLabel.text = _rei.reiOrderedAt;
        }
            break;
        case 3:
        {
            cell.textLabel.text = [NSString stringWithFormat:@"Received by: %@ %@", _rei.receiver.person.perName, _rei.receiver.person.perLastname];
