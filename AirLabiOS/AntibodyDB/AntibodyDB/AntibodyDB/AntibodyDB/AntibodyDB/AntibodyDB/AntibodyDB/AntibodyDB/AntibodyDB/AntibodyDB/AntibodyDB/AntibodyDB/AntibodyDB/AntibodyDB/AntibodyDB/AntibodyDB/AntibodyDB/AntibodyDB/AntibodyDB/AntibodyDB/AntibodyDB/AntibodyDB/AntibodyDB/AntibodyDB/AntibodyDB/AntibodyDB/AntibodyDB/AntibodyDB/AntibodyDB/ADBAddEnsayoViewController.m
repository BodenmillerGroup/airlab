...
-(void)done{
    if (_titleField.text.length == 0) {
        [General showOKAlertWithTitle:@"Please add a title" andMessage:nil];
        return;
    }else{
        if (_titleField.text.length > 200) {
            [General showOKAlertWithTitle:@"Title is too long" andMessage:nil];
        }
    }
    
    if (_purpose.text.length == 0) {
        [General showOKAlertWithTitle:@"Please add a purpose" andMessage:nil];
        return;
    }
    
    if (_hypothesis.text.length == 0) {
        [General showOKAlertWithTitle:@"Please add a hypothesis" andMessage:nil];
        return;
    }
