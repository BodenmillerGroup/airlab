...
- (void)viewDidLoad
{
    ...
    [General iPhoneBlock:nil iPadBlock:^{
        self.navigationItem.rightBarButtonItems = @[[[UIBarButtonItem alloc]initWithBarButtonSystemItem:UIBarButtonSystemItemEdit target:self action:@selector(edit)], [[UIBarButtonItem alloc]initWithBarButtonSystemItem:UIBarButtonSystemItemRefresh target:self action:@selector(reset)]];
    }];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.row == 1){
        ...
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(15, 10, cellSp.frame.size.width, [General calculateHeightedLabelForLabelWithText:_recipe.rcpPurpose andWidth:cellSp.frame.size.width andFontSize:17])];
        label.tag = 1;
