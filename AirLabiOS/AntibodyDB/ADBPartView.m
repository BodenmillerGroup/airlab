...
- (id)initWithFrame:(CGRect)frame andPart:(Part *)part
{
    ...
    if (self) {
        ...
        if ([part.prtType isEqualToString:@"pic"]) {
            ...
            __block NSData *imageData = [[(ZFilePart *)part.fileparts.allObjects.lastObject file] obtainZetData];
            if (imageData) {
        ...
        if ([part.prtType isEqualToString:@"fil"]) {
            if (part.zetUnscrolled.boolValue) {
                self.controlButton = [UIButton buttonWithType:UIButtonTypeCustom];
                __block NSData *file = [[(ZFilePart *)part.fileparts.allObjects.lastObject file]obtainZetData];
                if (file) {
...
-(void)dataToSave:(NSData *)data{
    ...
    file.zetData = data;
    file.catchedInfo = [NSString stringWithFormat:@"%@|%i", file.filHash, data.length];
    file.filSize = [NSString stringWithFormat:@"%i", data.length];
